...
int has_cycle(node *head) {
    ...
    //2 pointers at head of list
    node *first = head;
    node *second = head;

    //iterate
    while (second != NULL && second->next != NULL){
        //increment one pointer by 1 node
        first = first->next;
        //increment second pointer by 2 nodes
        second = second->next->next;

        //if not possible to one or the other due to a null pointer = end of a list
        //cycle if 2nd pointer same as first
        //if next node of second pointer = pointed by first pointer
        if (first == second || (second != NULL && second->next == first)){
            return 1;
        }
    }
